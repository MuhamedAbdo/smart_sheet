  ...
  void _compose() {
    ...
    for (int decompPos = compPos; decompPos < text.length; ++decompPos) {
      ...
      if ((composeType == null || (isShaddaPair)) &&
        ...
        // PATCH: guard against compPos exceeding lengths bounds
        if (starterPos < lengths.length) {
          lengths[starterPos] = lengths[starterPos] + 1;
        }
        ...
        lastClass = chClass;
        // PATCH: guard against compPos exceeding text/lengths bounds
        if (compPos < text.length) {
          text[compPos] = ch;
        } else {
          text.add(ch);
        }
        ...
        int chkPos = compPos;
        // PATCH: ensure lengths list is large enough before accessing
        while (lengths.length <= chkPos) {
          lengths.add(0);
        }
        if (lengths[chkPos] < 0) {
          while (chkPos < lengths.length && lengths[chkPos] < 0) {
            ...
            chkPos++;
            // PATCH: re-guard after insert may shift indices
            while (lengths.length <= chkPos) {
              lengths.add(0);
            }
          }
    ...
    // PATCH: clamp compPos to actual text length to avoid truncation errors
    if (compPos > text.length) compPos = text.length;
    text.length = compPos;
