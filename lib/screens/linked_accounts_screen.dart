...
class _LinkedAccountsScreenState extends State<LinkedAccountsScreen> {
  ...
  Future<void> _fetchLinkedAccounts() async {
    ...
    try {
      ...
      for (var w in workersRes) {
        if (w['email'] != null) {
          namesMap[w['email'].toString().toLowerCase().trim()] =
              w['name'].toString();
        }
      }
      ...
      if (mounted) {
        UIUtils.showInfoSnackBar(
            message: 'فشل تحميل الحسابات المرتبطة',
  ...
  Future<void> _unlinkAccount(String profileId) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'هل أنت متأكد من فك ارتباط هذا الحساب؟ سيتم طرده نهائياً.'),
        actions: [
          TextButton(
    ...
    try {
      await _supabase
          .from('profiles')
          .update({'factory_id': null}).eq('id', profileId);
      UIUtils.showInfoSnackBar(
          message: 'تم فك ارتباط الحساب بنجاح', backgroundColor: Colors.green);
      _fetchLinkedAccounts();
    } catch (e) {
      UIUtils.showInfoSnackBar(
          message: 'فشل فك الارتباط', backgroundColor: Colors.red);
    }
  }
  ...
  Future<void> _toggleSuspend(String profileId, String currentStatus) async {
    ...
    try {
      await _supabase
          .from('profiles')
          .update({'status': newStatus}).eq('id', profileId);
      UIUtils.showInfoSnackBar(
          message: 'تم $actionName الحساب بنجاح',
          backgroundColor: Colors.green);
      _fetchLinkedAccounts();
    } catch (e) {
      UIUtils.showInfoSnackBar(
          message: 'فشل $actionName الحساب', backgroundColor: Colors.red);
    }
  }
  ...
  Future<void> _showManualLinkDialog() async {
    if (!Hive.isBoxOpen('workers')) {
      UIUtils.showInfoSnackBar(
          message: 'سجل العمال غير متاح حالياً.', backgroundColor: Colors.red);
      return;
    }
    ...
    if (workers.isEmpty) {
      UIUtils.showInfoSnackBar(
          message: 'لا يوجد عمال مسجلين للربط.', backgroundColor: Colors.red);
      return;
    }
        ...
        builder: (ctx) {
          return StatefulBuilder(
            builder: (context, setStateSB) {
              return AlertDialog(
                title: const Text('ربط عامل بالمصنع (عبر حسابه)',
                    textDirection: TextDirection.rtl),
                content: Column(
                  mainAxisSize: MainAxisSize.min,
                  crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    const Text('اختر العامل:',
                        textDirection: TextDirection.rtl),
                    ...
                    const Text('البريد الإلكتروني لحساب العامل:',
                        textDirection: TextDirection.rtl),
                    const SizedBox(height: 8),
                    TextField(
                  ...
                  ElevatedButton(
                    onPressed: () {
                      ...
                      if (selectedWorker == null) {
                        UIUtils.showInfoSnackBar(
                            message: 'يرجى اختيار العامل.',
                            backgroundColor: Colors.red);
                        return;
                      }
                      if (accountEmail.isEmpty) {
                        UIUtils.showInfoSnackBar(
                            message: 'يرجى إدخال البريد الإلكتروني للحساب.',
                            backgroundColor: Colors.red);
                        return;
                      }
  ...
  Future<void> _linkWorkerByEmail(String accountEmail, Worker worker) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'هل أنت متأكد من ربط حساب "$accountEmail"\nبالعامل "${worker.name}" في هذا المصنع؟\nسيتم السماح له بتسجيل الدخول فوراً.',
            textDirection: TextDirection.rtl),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
    ...
    try {
      ...
      if (profileRes != null) {
        ...
        await _supabase
            .from('profiles')
            .update({'factory_id': currentFactoryId, 'status': 'active'}).eq(
                'id', profileRes['id']);
      }
      ...
      UIUtils.showInfoSnackBar(
          message: 'تم ربط الحساب والعامل بنجاح',
          backgroundColor: Colors.green);
      ...
      UIUtils.showInfoSnackBar(
          message: 'حدث خطأ أثناء الربط: $e', backgroundColor: Colors.red);
    }
  }

  Future<void> _managePermissions(
      String profileId, String role, String? email, String? workerName) async {
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
                'لإدارة صلاحيات هذا الحساب، نحتاج لإنشاء سجل عامل له (أو ربطه بسجل موجود عبر الإيميل).'),
            const SizedBox(height: 10),
            TextField(
              controller: nameController,
              decoration: const InputDecoration(
                  labelText: 'اسم الجهاز/الحساب (مثال: جهاز السكرتارية)'),
            ),
            const SizedBox(height: 10),
            TextField(
              controller: emailController,
              readOnly:
                  email != null && email != 'غير متوفر' && email.isNotEmpty,
              decoration:
                  const InputDecoration(labelText: 'البريد الإلكتروني للحساب'),
            ),
          ],
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                final existingWorkers = await _supabase
                    .from('workers')
                    .select()
                ...
                UIUtils.showInfoSnackBar(
                    message: 'حدث خطأ: $e', backgroundColor: Colors.red);
              }
            },
  ...
  Future<void> _openWorkerFormDirectly(String email) async {
    ...
    try {
      final existingWorkers =
          await _supabase.from('workers').select().eq('email', email.trim());
      Worker workerObj;
      final box = Hive.box<Worker>('workers');
      ...
      if (existingWorkers.isNotEmpty) {
        ...
        if (mounted) Navigator.pop(context);
        UIUtils.showInfoSnackBar(
            message: 'لم يتم العثور على سجل العامل',
            backgroundColor: Colors.red);
      }
    } catch (e) {
      if (mounted) Navigator.pop(context);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('ربط حساب بعامل',
          ...
          child: Container(
            ...
            child: ConstrainedBox(
              ...
              child: _isLoading
                  ...
                  : _profiles.isEmpty
                      ? const Center(
                          child: Text('لا توجد حسابات مرتبطة حالياً.'))
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final email =
                                profile['email']?.toString() ?? 'غير متوفر';
                            final workerName =
                                _workerNames[email.toLowerCase().trim()];
                                ...
                                child: Container(
                                  ...
                                  child: ListTile(
                                    ...
                                    title: Text(workerName ??
                                        'حساب غير مسمى (ID: ${profile['id'].toString().substring(0, 8)})'),
                                    subtitle: Column(
                                      crossAxisAlignment:
                                      ...
                                      children: [
                                        ...
                                        Text(
                                            'الدور: ${role == 'admin' ? 'مدير' : 'جهاز مرتبط'}'),
                                        const SizedBox(height: 4),
                                        Text(
                                          isSuspended
                                              ? 'الحالة: موقوف مؤقتاً'
