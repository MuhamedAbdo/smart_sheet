...
class _SettingsScreenState extends State<SettingsScreen> {
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: isAdmin
                  ? () => _pickAndUploadLogo(context, themeProvider)
                  : null,
            ),
            const Divider(),
            ...
            ListTile(
              ...
              subtitle: Text(themeProvider.printedFactoryName,
                  maxLines: 2, overflow: TextOverflow.ellipsis),
              trailing: isAdmin ? const Icon(Icons.edit, size: 18) : null,
              onTap: isAdmin
                  ? () => _editPrintedFactoryName(context, themeProvider)
                  : null,
            ),
          ],
        ),
  ...
  Future<void> _editPrintedFactoryName(
      BuildContext context, ThemeProvider themeProvider) async {
    final controller =
        TextEditingController(text: themeProvider.printedFactoryName);
        ...
        try {
          ...
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content: Text('تم رفع إسم المصنع بنجاح!'),
          ...
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content: Text('فشل الرفع للسحابة: $e'),
  ...
  Future<void> _pickAndUploadLogo(
    ...
    try {
          ...
          if (originalImage != null) {
            ...
            if (factoryId != null) {
              try {
                ...
                debugPrint(
                    "❌ [factories] فشل رفع الشعار للسحابة (قد يكون العمود مفقوداً): $dbError");
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text(
                            'تم حفظ الشعار محلياً، لكن فشل الرفع للسحابة (يرجى إضافة العمود)'),
            ...
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                    content: Text('تم حفظ الشعار بنجاح!'),
                    backgroundColor: Colors.green),
              );
            }
          }
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('فشل اختيار الشعار'), backgroundColor: Colors.red),
        );
      }
    }
  }
