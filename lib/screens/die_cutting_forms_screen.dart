...
class _DieCuttingFormsScreenState extends State<DieCuttingFormsScreen> {
            ...
            child: ValueListenableBuilder(
              ...
              builder: (context, Box<DieCuttingForm> box, _) {
                if (box.values.isEmpty) {
                  return const Center(child: Text("لا توجد قوالب تكسير مسجلة", style: TextStyle(fontSize: 18)));
                }
                ...
                if (forms.isEmpty) {
                  return const Center(child: Text("لا توجد قوالب تطابق البحث", style: TextStyle(fontSize: 18)));
                }
                                ...
                                children: [
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text("رقم الفورمة: ${form.formNumber}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                  ...
                                  if (form.isSheet) const Chip(label: Text('شيت', style: TextStyle(fontSize: 10))),
                                ],
                              ),
                              Row(
                                mainAxisAlignment: MainAxisAlignment.end,
                                children: [
                                  IconButton(icon: const Icon(Icons.edit, color: Colors.orange), onPressed: () => _showFormDialog(context, form: form)),
                                  IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _confirmDelete(context, form)),
                                ],
                              ),
                            ],
                          ),
  ...
  void _showFormDialog(BuildContext context, {DieCuttingForm? form}) {
    showDialog(context: context, builder: (context) => _DieCuttingFormDialog(form: form));
  }
...
class _DieCuttingFormDialogState extends State<_DieCuttingFormDialog> {
  ...
  void initState() {
    ...
    _lengthController = TextEditingController(text: widget.form?.length != null ? widget.form!.length.toString() : '');
    _widthController = TextEditingController(text: widget.form?.width != null ? widget.form!.width.toString() : '');
    _heightController = TextEditingController(text: widget.form?.height != null ? widget.form!.height.toString() : '');
    _sheetLengthController = TextEditingController(text: widget.form?.sheetLength != null ? widget.form!.sheetLength.toString() : '');
    _sheetWidthController = TextEditingController(text: widget.form?.sheetWidth != null ? widget.form!.sheetWidth.toString() : '');
    _numberOfBoxesController = TextEditingController(text: widget.form?.numberOfBoxes != null ? widget.form!.numberOfBoxes.toString() : '');
  ...
  @override
  void dispose() {
    for (final c in [_formNumberController, _lengthController, _widthController, _heightController,
        _sheetLengthController, _sheetWidthController, _numberOfBoxesController, _shelfLocationController,
        _customerNameController, _customerCodeController, _itemNameController, _itemCodeController]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  void _onCustomerSelected(String? customerName) {
    ...
    for (final entry in box.toMap().entries) {
      ...
      if (val is! Map) continue;
      if ((val['clientName']?.toString().trim() ?? '') != customerName) continue;
      if (val['processType']?.toString() != 'تكسير') continue;
      if (val['isClientRecord'] == true) continue;
      
      final identifier = val['id']?.toString() ?? val['productCode']?.toString() ?? entry.key.toString();
      uniqueItems[identifier] = {...Map<String, dynamic>.from(val), '__hiveKey': entry.key};
    }
  ...
  void _save() {
    ...
    if (widget.form == null && _selectedItemHiveKey != null && Hive.isBoxOpen('savedSheetSizes')) {
      ...
      if (existing is Map) {
        ...
        SyncService.instance.pushToQueue('customers', updatedItem, operation: 'upsert');
        debugPrint('✅ [DieCuttingForms] Two-Way Save: تحديث formNumber في سجل العميل'
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                if (!isEditing) ...[
                  OutlinedButton.icon(
                    icon: Icon(_showCustomerLookup ? Icons.expand_less : Icons.manage_search, size: 18),
                    label: Text(_showCustomerLookup ? 'إخفاء استدعاء سجل العملاء' : 'استدعاء بيانات من سجل العملاء'),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            items: _allCustomerNames.map((name) => DropdownMenuItem(value: name, child: Text(name, overflow: TextOverflow.ellipsis))).toList(),
                          ...
                          if (_selectedCustomerName != null) ...[
                            ...
                            DropdownButtonFormField<Map<String, dynamic>>(
                              ...
                              hint: _customerItems.isEmpty
                                  ? const Text('لا توجد أصناف تكسير لهذا العميل', style: TextStyle(fontSize: 13))
                                  : const Text('اختر صنفاً...'),
                              items: _customerItems.map((item) => DropdownMenuItem(
                                value: item,
                                child: Text('${item['productName'] ?? '—'}  (${item['productCode'] ?? ''})', overflow: TextOverflow.ellipsis),
                              )).toList(),
                              onChanged: _customerItems.isEmpty ? null : _onItemSelected,
                            ),
                          ],
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: _itemHasNoFormNumber ? const Icon(Icons.edit_note, color: Colors.orange) : null,
                  ),
                  validator: (v) => (v == null || v.isEmpty) ? 'رقم الفورمة مطلوب' : null,
                ),
                const SizedBox(height: 10),
                ...
                Row(children: [
                  Expanded(child: TextFormField(controller: _customerNameController, decoration: const InputDecoration(labelText: 'اسم العميل (اختياري)', border: OutlineInputBorder()))),
                  const SizedBox(width: 8),
                  Expanded(child: TextFormField(controller: _customerCodeController, decoration: const InputDecoration(labelText: 'كود العميل (اختياري)', border: OutlineInputBorder()))),
                ]),
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: TextFormField(controller: _itemNameController, decoration: const InputDecoration(labelText: 'اسم الصنف (اختياري)', border: OutlineInputBorder()))),
                  const SizedBox(width: 8),
                  Expanded(child: TextFormField(controller: _itemCodeController, decoration: const InputDecoration(labelText: 'كود الصنف (اختياري)', border: OutlineInputBorder()))),
                ]),
                const SizedBox(height: 10),

                TextFormField(
                  controller: _numberOfBoxesController,
                  decoration: const InputDecoration(labelText: 'عدد العلب', border: OutlineInputBorder()),
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'مكان الرف (اختياري)', border: OutlineInputBorder()),
                ),
                const SizedBox(height: 4),
