...
class _BackupRestoreScreenState extends State<BackupRestoreScreen> {
  ...
  Future<void> _openQRScanner() async {
    ...
    if (isMobilePlatform) {
      try {
        // منطق الهواتف (Android/iOS): فتح شاشة قارئ الباركود مباشرة
        result = await _showQRScannerDialog();
      } catch (e) {
        // في حال حدوث خطأ في تشغيل الكاميرا (مثل الحاجة لإعادة بناء التطبيق Native Build)
        result = 'MANUAL_ENTRY';
      }
      // في حال الضغط على زر "إدخال الكود يدوياً" أسفل شاشة الكاميرا أو حدوث خطأ
      if (result == 'MANUAL_ENTRY') {
        if (!mounted) return;
        result = await _showManualEntryDialog();
      }
    } else {
      // منطق الديسكتوب والويب: تخطي الكاميرا وفتح نافذة إدخال الكود مباشرة
      result = await _showManualEntryDialog();
    }
